...
The whole app is shell and JavaScript — nothing to compile, and everything under
`package/files/` is installed as-is. The assertions covering the duration parser, the backoff
thresholds, the URL encoding and the password masking ship inside the binary itself:

```bash
sh package/files/usr/sbin/watchplug selftest
```

They need no dependency beyond a POSIX shell, which is why `network.sh` is sourced inside
`resolve_wan()` rather than at the top of the file.

```bash
sh test-daemon.sh
```

runs the actual daemon loop against a stubbed `uci`, `ping` and HTTP device, with timings
compressed to seconds instead of minutes (~30 s). It covers the
`ok → degraded → cycle → cooldown → blocked → rearm → ok` sequence, the `Backlog` command sent to
the plug, the outage clock surviving a daemon restart, and that neither preset ever writes the
device password into the activity log. CI runs both on every push.

```bash
sh make-ipk.sh
```

builds the `.ipk` without the OpenWrt SDK — there is nothing to cross-compile. It numbers the
package `1.0.0-<commit count>`, so every commit produces a version opkg accepts as an upgrade with
nothing to bump by hand; `VERSION` and `RELEASE` override it. **Commit before building**, or the
artifact carries the previous number.

Pushing a `v*` tag is what publishes a release: CI runs the selftest, builds the package with
`VERSION` taken from the tag, and attaches the `.ipk` to the release. Every other push uploads it
